	...
	try{
		...
		async function test(){
			...
			if (page.includes('log_out')) {
				...
				if(asset==true){
				...
				else if(typeof token !== 'undefined' || typeof company !== 'undefined'){
					...
					if (page.includes('/api/transaction')) {
						let bod='';
						if(method=='GET'){
											  ...
											  res.on('end', () => {
												...
												try{
													if(bod.length==128){
														...
														console.log("cook "+cook);
														session.set(cook,'{"ip":"'+remote_ip+'","entete":"'+requete.headers['user-agent']+'","token":"'+bod+'","domaine":"'+output.company.toLowerCase()+'","time":'+Date.now()+'}');
...
async function getquery3(dataset,cookie,ind) {
	...
	async function post_interval(){
		...
		try{
			...
			console.log('json  '+JSON.stringify(bo));
